...
/**
 * Slug plus the date the sitemap should report as `lastmod`, for the same set
 * of entries. `updatedAt` when the file has one, `publishedAt` otherwise —
 * the frontmatter, never the build clock. See app/sitemap.js.
 */
export function getPublishedProjectEntries() {
  return getAllProjects()
    .filter(isPublished)
    .map((p) => ({ slug: p.slug, updated: p.updatedAt || p.publishedAt }));
}
